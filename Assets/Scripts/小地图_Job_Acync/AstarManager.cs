...
namespace 小地图_Job_Acync
{
    ...
    public class AStarManager : Singleton<AStarManager>
    {
        ...
        /// 直观查看寻路消耗
        /// </summary>
        public NativeList<int> visited = new NativeList<int>(10000, Allocator.Persistent);
        /// <summary>
        /// 最终路径的列表
        /// </summary>
        public NativeList<int> pathList = new NativeList<int>(10000, Allocator.Persistent);
        ...
        public void Initialize(int width, int height)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    AStarNode node = new AStarNode((byte)x, (byte)y,
        ...
        public JobHandle? FindPathAsync(Vector3 startPos, Vector3 endPos)
        {
            // int count = 0;
            //首先要判断 传入的坐标是否合法
            //1.要在地图范围你
            ...
            int startY = (int)(startPos.z / lengthOfSide);
            //判断是否超出二维数组索引
            int endX = (int)(endPos.x / lengthOfSide);
            int endY = (int)(endPos.z / lengthOfSide);
            ...
            //2.不要是阻挡地图
            //判断起点和终点是否是阻挡地图
            if (nodeArrayNative[endNodeIndex].type == AStarNodeType.Stop)
            {
        ...
        void SetFatherIndex(int index, int fatherIndex)
        {
            ...
            node.fatherIndex = (short)fatherIndex;
